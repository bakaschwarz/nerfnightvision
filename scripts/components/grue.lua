-- Modified Charlie to attack players using nightvision
...
function Grue:CheckForStart()
    return not (self.inst.components.health:IsInvincible() or
                ...
                Grue:ExcludeWoodie(self.inst))
end

function Grue:ExcludeWoodie(inst)
    if inst.prefab == "woodie" then
        if TUNING.nerfednightvision.Woodie == "TRUE" then
